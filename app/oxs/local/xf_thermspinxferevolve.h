/* FILE: xf_thermspinxferevolve.h                  -*-Mode: c++-*-
 *
...
#ifndef _XF_THERMSPINXFEREVOLVE
#define _XF_THERMSPINXFEREVOLVE
...
class Xf_ThermSpinXferEvolve:public Oxs_TimeEvolver {
private:
  ...
  // Added to include thermal simulations
  const OC_REAL8m KBoltzmann;            // Boltzmann constant
  mutable Oxs_MeshValue<ThreeVector> Hfluct; // Mesh of vectors with standard Gaussian random numbers as their components
  mutable Oxs_MeshValue<OC_REAL8m> h_fluctVarConst; // Variance of thermal fluctuation (at each mesh point) without temperature factor
  mutable OC_REAL8m temperature; // temperature in Kelvin
  mutable OC_REAL8m tmpStepsize; // For passing step sizes internally
  ...
  **/
  ...
  OC_REAL8m EvaluateTemperatureProfileScript(OC_UINT4m stage, OC_REAL8m stage_time,
					     OC_REAL8m total_time) const;
  ...
  **/
  ...
  void UpdateHfluct(const Oxs_Mesh*);
  void InitHfluct(const Oxs_Mesh*);
  void FillHfluctConst(const Oxs_Mesh*);
  ...
  void UpdateTemperatureOutputs(const Oxs_SimState&);
  void UpdateSpinTorqueOutputs(const Oxs_SimState&);
  Oxs_ScalarOutput<Xf_ThermSpinXferEvolve> max_dm_dt_output;
  Oxs_ScalarOutput<Xf_ThermSpinXferEvolve> dE_dt_output;
  Oxs_ScalarOutput<Xf_ThermSpinXferEvolve> delta_E_output;
  Oxs_ScalarOutput<Xf_ThermSpinXferEvolve> temperature_output;
  Oxs_ScalarOutput<Xf_ThermSpinXferEvolve> ave_J_output;
  Oxs_VectorFieldOutput<Xf_ThermSpinXferEvolve> dm_dt_output;
  Oxs_VectorFieldOutput<Xf_ThermSpinXferEvolve> mxH_output;
  Oxs_VectorFieldOutput<Xf_ThermSpinXferEvolve> spin_torque_output;
  Oxs_VectorFieldOutput<Xf_ThermSpinXferEvolve> Jmp_output;
...
#define RKStepFuncSig(NAME) \
  ...
  RKStepFuncSig((Xf_ThermSpinXferEvolve::* rkstep_ptr));
  ...
  Xf_ThermSpinXferEvolve(const Xf_ThermSpinXferEvolve&);
  Xf_ThermSpinXferEvolve& operator=(const Xf_ThermSpinXferEvolve&);

public:
  virtual const char* ClassName() const; // ClassName() is
  /// automatically generated by the OXS_EXT_REGISTER macro.
  virtual OC_BOOL Init();
  Xf_ThermSpinXferEvolve(const char* name,     // Child instance id
		       Oxs_Director* newdtr, // App director
		       const char* argstr);  // MIF input block parameters
  virtual ~Xf_ThermSpinXferEvolve();
